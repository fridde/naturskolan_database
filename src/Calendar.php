    ...
    public function update_calendar_db()
{
  $settings = json_decode(file_get_contents('settings.json'), true);
        ...
        $locationTranslator = array('2/3' => $settings['locationTranslator2'], '5' => $settings['locationTranslator5']);

        $groupTable = sql_select('grupper');
        $teacherTable = sql_select('larare', array('status' => 'active'));
        ...
        $insertArray = array();
            ...
            foreach ($group as $columnName => $cell) {
                ...
                if ($cell != '' && in_array($columnName, $visitArray)) {
                    ...
                    $info = trim($lararRow['info']);
                    if ($info) {
                        $description[] = 'Annat: '.$info;
                    }
                    $notes = trim($lararRow['notes']);
                    if ($notes) {
    ...
    public function blur_mail($string)
    {
